...
import com.chiendang.cooking.utils.PostStatus;
...
public class RecipeServiceImpl implements RecipeService {
    ...
    @Override
    public List<RecipeResponse> getAllRecipeApproved (){

        List<Recipe> recipes = recipeRepository.findByStatus(PostStatus.APPROVED);
    ...
    // Phương thức duyệt bài viết
    public void approveRecipe(Integer recipeId) {
        Recipe recipe = recipeRepository.findById(recipeId)
                .orElseThrow(() -> new AppExceptions(ErrorCode.RESOURCES_NOT_FOUND));

        recipe.setStatus(PostStatus.APPROVED);
    ...
    public void rejectRecipe(Integer recipeId) {
        ...
        recipe.setStatus(PostStatus.REJECTED);
        recipeRepository.save(recipe);
    }

    // Phương thức lấy các bài viết chưa duyệt
    @Override
    public List<RecipeResponse> getPendingRecipes() {
        return  recipeRepository.findByStatus(PostStatus.PENDING).stream()
                .map(recipeMapper::toRecipeResponse).toList();
    }
